...
class Trainer():
    ...
    def update(self, model, batch, step, eval=False):
        ...
        b_im_r = b_im.reshape(bs*img_stack_len, 3, 224, 224) # (bs*5, 3, 224, 224) or 180 320
        ...
        if model.module.tcnweight > 0:
            ...
            for _ in range(num_neg_v):
                es0_shuf = es0[torch.randperm(es0.size()[0])] # shuffle, bs, 1, embedding_shape
            ...
            a_state = ((1.0 * (sim_0_2 < sim_1_2)) * (1.0 * (sim_0_1 > sim_0_2))).mean() # % of correctly aligned sample pairs
        ...
        if model.module.align_state_weight > 0:
            ...
            assert s0_proj.shape == es0.shape, f"img embedding and statewind embedding shape mismatch, {es0.shape} and {s0_proj.shape}"
            ...
            s0loss = -torch.log(epsilon + (torch.exp(sim_0_0s) / (epsilon + torch.exp(sim_0_0s) + torch.exp(sim_0_2s))))
            s2loss = -torch.log(epsilon + (torch.exp(sim_2_2s) / (epsilon + torch.exp(sim_2_2s) + torch.exp(sim_2_0s))))
            state_align_loss = ((s0loss + s2loss) / 2.0).mean()
            a_img_state = ((1.0 * (sim_0_2s < sim_0_0s)) * (1.0 * (sim_2_0s < sim_2_2s))).mean()
